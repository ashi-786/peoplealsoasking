...
async def extract_paa_questions(page, parent_kw, max_questions=4):
    questions_data = []
    try:
        await page.wait_for_selector('div[jsname="yEVEwb"]', timeout=7000)
    except PlaywrightTimeoutError:
        return []

    boxes = await page.query_selector_all('div[jsname="yEVEwb"]')
    ...
    boxes = (await page.query_selector_all('div[jsname="yEVEwb"]'))[:max_questions]

    for box in boxes:
        try:
            question_el = await box.query_selector('div.related-question-pair')
            answer_el = await box.query_selector('div[data-attrid], span.hgKElc')
            link_el = await box.query_selector('a')
            title_el = await link_el.query_selector('h3')

            question = await question_el.get_attribute("data-q") if question_el else None
            answer = (await answer_el.inner_text()).strip() if answer_el else None
...
async def scrape_single_query(context, query, max_questions=4):
    ...
    try:
        ...
        questions = await extract_paa_questions(page, parent_kw=query, max_questions=max_questions)
        return questions
    except PlaywrightTimeoutError:
...
async def scrape_single_query_with_retry(context, query, max_questions, retries=2):
    for attempt in range(retries):
        try:
            return await scrape_single_query(context, query, max_questions)
        except PlaywrightTimeoutError:
...
async def scrape_main_keyword(context, main_kw, max_depth=3, max_questions=4):
    ...
    for depth in range(1, max_depth + 1):
        print(f"Scraping depth {depth} with {len(current_level_queries)} queries...")
        # Run all queries at this depth concurrently
        tasks = [scrape_single_query_with_retry(context, q, max_questions) for q in current_level_queries]
        ...
        if not next_level_queries:
            ...
            break  # Stop if no new questions
...
async def scrape_google_paa(main_kw):
    ...
    async with async_playwright() as playwright:
        ...
        main_kw_obj = await sync_to_async(save_results_to_db)(main_kw, results)
...
# main_kw_obj = asyncio.run(scrape_google_paa("affordable web hosting")) # accepts maximum 3 words as main keyword